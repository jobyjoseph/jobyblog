...
We'll solve this by:
1. Customizing Shopify's email templates
...
2. Find the password reset email from Shopify
3. Verify the reset link points to your domain (not `myshopify.com`)
4. Example: `https://your-site.com/account/reset-password?url=...`

### 3. Test Password Reset

1. Click the reset link in the email
2. Verify you land on `/account/reset-password`
3. Enter a new password (minimum 5 characters)
4. Confirm the new password
5. Click "Reset Password"
6. Verify success message appears
7. Verify automatic redirect to homepage
8. Verify you're automatically signed in
...
- **Expired reset link**: Should show "Unable to reset password"
- **Passwords don't match**: Should show "Passwords do not match"
- **Password too short**: Should show "Password must be at least 5 characters"
- **Missing URL parameter**: Should show "Invalid or missing reset link"

## Security Best Practices

### 1. Use HTTPS in Production

Always use HTTPS for your production site. The reset URLs contain sensitive tokens.
...
Store access tokens in HTTP-only cookies to prevent XSS attacks:
...
Consider implementing rate limiting for the forgot password endpoint to prevent abuse:

```typescript
// Example with express-rate-limit (if using custom API routes)
...
Shopify's reset tokens have built-in expiration. The `customerResetByUrl` mutation will fail if the token is expired, which is handled by our error handling:
...
### Issue 1: Email Still Points to Shopify Store

**Problem:** Reset emails still redirect to `your-store.myshopify.com`

**Solution:**
1. Double-check your Shopify notification template
2. Ensure you've saved the template changes
3. Test with a new password reset request (old emails use old template)

### Issue 2: "Invalid reset link" Error

**Problem:** URL parameter is not being parsed correctly

**Solution:**
1. Verify the URL parameter is named `url`
2. Check URL encoding: `{{ customer.reset_password_url | url_encode }}`
3. Ensure `useSearchParams()` is correctly extracting the parameter

### Issue 3: Types Not Generated

**Problem:** TypeScript errors for `CustomerResetByUrlDocument`

**Solution:**
```bash
# Run codegen after creating GraphQL files
npm run codegen
```

### Issue 4: Cookie Not Set After Reset

**Problem:** User not automatically signed in after password reset

**Solution:**
1. Check that `customerResetByUrl` returns an access token
2. Verify cookie settings (domain, path, secure)
3. Check browser developer tools → Application → Cookies

### Issue 5: CORS Errors

**Problem:** CORS errors when calling Better-Auth endpoints

**Solution:**
Better-Auth handles CORS automatically. If you still see errors:
1. Ensure you're calling from the same domain
2. Check Next.js API route configuration
3. Verify Better-Auth initialization in `auth.ts`
...
1. User enters email
   ↓
2. Client calls authClient.shopifyForgotPassword()
   ↓
3. Better-Auth endpoint /shopify-auth/forgot-password
   ↓
4. Calls customerRecover mutation
   ↓
5. Shopify sends email with custom URL
   ↓
6. Email contains: your-site.com/reset-password?url=shopify-url
...
1. User clicks email link
   ↓
2. Lands on /account/reset-password with URL param
   ↓
3. User enters new password
   ↓
4. Client calls authClient.shopifyResetPassword()
   ↓
5. Better-Auth endpoint /shopify-auth/reset-password
   ↓
6. Calls customerResetByUrl mutation
   ↓
7. Shopify returns success + access token
   ↓
8. Better-Auth sets cookie (auto sign-in)
   ↓
9. User redirected to homepage (authenticated)
```
...
You now have a complete, secure password recovery system for your headless Shopify storefront! The implementation:
